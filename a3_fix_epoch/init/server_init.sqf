...
_abortAndError={publicVariable "EPOCH_SERVER";
EPOCH_BADHIVE=true;
publicVariable "EPOCH_BADHIVE";
for "_i" from 0 to 15 do{diag_log _this;
};
};
...
if(_clientVersion !=Epoch_ServerVersion)exitWith{format["Epoch: Version mismatch! Current: %2 Needed: %1",_clientVersion,Epoch_ServerVersion]call _abortAndError;
};
...
if(isNil "_instanceID")exitWith{"Epoch: InstanceID missing!" call _abortAndError;
};
...
onPlayerDisconnected{diag_log format["playerDisconnected:%1:%2",_uid,_name];
...
if(isClass _epochWorldPath && isNumber(_epochWorldPath >> "worldSize"))then{_worldSize=getNumber(_epochWorldPath >> "worldSize");
};
epoch_centerMarkerPosition=getMarkerPos "center";
if(epoch_centerMarkerPosition isEqualTo[0,0,0])then{diag_log "Epoch: Error cannot find center marker!";
};
EPOCH_dynamicVehicleArea=_worldSize / 2;
diag_log "Epoch: Set Weather";
true call EPOCH_server_setWeather;
EPOCH_customChannels=[];
for "_i" from 0 to 9 do{_radio=configfile >> "CfgWeapons" >> format["EpochRadio%1",_i];
...
_startTime spawn{diag_log "Epoch: Loading buildings";
...
if(_staticDateTime isEqualto[])then{_response="epochserver" callExtension "510";
if(_response !="")then{diag_log format["Epoch: Set Real Time: %1",_response];
...
}else{{if(_x !=0)then{_date set[_forEachIndex,_x];
_dateChanged=true;
};
}forEach _staticDateTime;
};
if(_dateChanged)then{setDate _date;
};
